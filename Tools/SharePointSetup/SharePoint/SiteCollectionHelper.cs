...
using CAS.SharePoint.Setup.Properties;
using Microsoft.SharePoint;
using Microsoft.SharePoint.Administration;

namespace CAS.SharePoint.Setup.SharePoint
{
  internal class SiteCollectionHelper: IDisposable
  {

    #region creator
    internal SiteCollectionHelper( SPWebApplication _wapplication, string _Url )
    {
      try
      {
    ...
    internal static SiteCollectionHelper GetSPSite
      ( SPWebApplication _wapplication, string siteUrl, string title, string description, uint nLCID, string webTemplate, string ownerLogin, string ownerName, string ownerEmail )
    {
      try
      {
        if ( _wapplication.Sites.Names.Contains( siteUrl ) )
          ...
          {
            return new SiteCollectionHelper( _wapplication.Sites[ siteUrl ] ) { NewTemplateRequired = false };
          }
        return new SiteCollectionHelper( _wapplication.Sites.Add( siteUrl, title, description, nLCID, webTemplate, ownerLogin, ownerName, ownerEmail ) ) { NewTemplateRequired = String.IsNullOrEmpty( webTemplate ) };
      }
      catch ( Exception ex )
      {
    ...
    internal bool NewTemplateRequired { get; private set; }
    internal string SiteTemplate { get { return SiteCollection.RootWeb.WebTemplate; } }
    internal int SiteTemplateID { get { return SiteCollection.RootWeb.WebTemplateId; } }
    /// <summary>
